...
// Кеш провайдера — всі операції йдуть через один канал
...
async function connectMetaMaskSDK(): Promise<Eip1193Provider> {
  ...
  // конект до гаманця
  await _sdk.connect().catch(() => { /* користувач міг скасувати */ });
...
// ───────────────── WalletConnect v2 (deeplink на MetaMask) ─────────────────
// @ts-ignore — типи інколи сипляться у Vite
...
async function connectWalletConnect(): Promise<Eip1193Provider> {
  ...
  // на мобілці відправляємо deeplink metamask://wc?uri=...
  ...
  await provider.connect().catch((e: any) => { throw e; });
