...
/** Fallback через MetaMask SDK (коли deeplink не спрацював) */
let __sdk: any | null = null;
async function ensureMobileWalletReadyFallback() {
...
/**
 * ✅ Головний “жорсткий” підйом MetaMask:
 * 1) якщо інжект є — робимо eth_requestAccounts та перемикаємося на BSC
 * 2) якщо інжекту немає і це мобілка — відкриваємо MetaMask через deeplink,
 *    чекаємо повернення, знову робимо connect + switch
 * 3) fallback на MetaMask SDK, якщо deeplink не дав інжект
 */
async function ensureMMNow() {
  ...
  // 2) Нема інжекту → мобільний deeplink
  if (isMobileUA()) {
    const dappUrl = encodeURIComponent(window.location.href);
    // офіційний універсальний лінк MetaMask (надійніше за "metamask://")
    const mmLink = `https://metamask.app.link/dapp/${dappUrl}`;

    // позначка, що ми переходили в MM — щоб уникнути циклів
    sessionStorage.setItem('mm-opened', String(Date.now()));

    // переходимо в MetaMask
    window.location.href = mmLink;

    // чекаємо, поки користувач повернеться у вкладку (MetaMask показує “Return to app”)
    await waitUntilVisible(20000);

    // після повернення інжект має бути
    const eth2 = (window as any).ethereum;
    if (eth2 && (eth2.isMetaMask || eth2.request)) {
    ...
    // 3) Якщо все ж інжект не зʼявився — fallback SDK
    await ensureMobileWalletReadyFallback();
    return;
  }

  // 4) Десктоп без інжекту — пропонуємо встановити MetaMask
...
export default function MyOrders() {
  ...
  const handleLock = async (s: Scenario) => {
    ...
    try {
      // 1) Підіймаємо MetaMask (мобільний deeplink → connect → BSC)
      await ensureMMNow();
      ...
      if (setup?.approveTxHash) {
        // можемо показати тост, якщо потрібно
      }
