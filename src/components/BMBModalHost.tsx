...
/**
 * Легкий корпоративний модал-хост для BMB.
 * Показ/закриття через події:
 *   window.dispatchEvent(new CustomEvent('bmb:modal:open', { detail: { title, subtitle, kind, actionLabel, noBackdropClose, hideClose } }));
...
const WalletBadges = () => (
  <div style={{ display:"flex", gap:12, alignItems:"center", justifyContent:"center" }}>
    <img src="/MetaMask-icon-fox.svg" alt="" width={56} height={56} style={{ borderRadius:9999, border:"3px solid #fff", boxShadow:"0 2px 8px rgba(0,0,0,.15)" }}/>
    <img src="/BNB,_native_cryptocurrency_for_the_Binance_Smart_Chain.svg" alt="" width={56} height={56} style={{ borderRadius:9999, border:"3px solid #fff", boxShadow:"0 2px 8px rgba(0,0,0,.15)" }}/>
  </div>
);

const WalletDoneBadge = () => (
  <BadgeCircle>
    <img src="/mUSD-icon.svg" alt="" width={52} height={52} />
  </BadgeCircle>
);
...
function BMBModal({
  ...
  noBackdropClose, hideClose, icon,
  actionsSlot,
}: {
  open: boolean;
  onClose: () => void;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {title ? <h3 style={{ margin:"6px 0 4px", fontSize:22, fontWeight:800 }}>{title}</h3> : null}
        {subtitle ? <div style={{ margin:"0 auto 14px", maxWidth:560, opacity:.9 }}>{subtitle}</div> : null}
...
export default function BMBModalHost() {
  ...
  useEffect(() => {
    const onOpen = (e: Event) => {
      ...
      const next: Required<Payload> & { kind: Kind } = {
        kind: (detail.kind || "info") as Kind,
        title: detail.title ?? "Повідомлення",
        subtitle: detail.subtitle ?? "",
        actionLabel: detail.actionLabel ?? "Добре",
        noBackdropClose: !!detail.noBackdropClose,
        hideClose: !!detail.hideClose,
      };
      setState(next);
      setOpen(true);
    };
